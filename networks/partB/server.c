...
// function to send array of chunks to the client
...
// function to receive acknowledgement from the client
...
// function to resend chunks to the client, if acknowledgement is not received and timeout occurs
void *resend_chunks(void *args)
{
    ...
    for (int i = 0; i < num_chunks; i++)
    {
        ...
        // Resend chunk to the client if acknowledgement is not received and timeout occurs
...
void send_message()
{
    ...
    // Number of chunks to the client
    ...
    {
        printf("Number of chunks sent to the client are %d with length %ld\n", num_chunks, strlen(message));
    }
...
// function to receive message from the client
void receive_message()
{
    printf("Waiting for client to send message...\n");
    // Now, receive the message from the client
    int client_socket;
    struct sockaddr_in server_addr;
    socklen_t server_addr_len = sizeof(server_addr);
    ...
    // Receive message from the client about the number of chunks being transmitted
