...
// Used ChatGPT and Copilot for assistance.
    ...
    // Here, we iterate through all the processes in order to find those with 
    // either higher prioirity than current
    // or
    // those who are in same priority but entered before
    struct proc *min_proc = 0;
    for (p = proc; p < &proc[NPROC]; p++)
    {
      acquire(&p->lock);
